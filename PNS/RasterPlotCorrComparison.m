...
%             end
            ...
            if ~isempty(trialSpikes)
                ...
                plot(H, (xx - (buffer/fs) + (k-1)*trialSize/fs + (j-1)*trialSize/fs*20), yy, spikeColor{j}, 'linewidth', spikeWidth)
...
% Styles plot
xlim([-buffer/fs, 3*(numTrials*trialSize)/fs])
ylim([0, numChans*10])

if flagLabel
    ylabel('Electrode', 'fontsize', 15)
    xlabel(sprintf('Trials (%d sec/trial)', trialLen/fs), 'fontsize', 15)
end % END IF
