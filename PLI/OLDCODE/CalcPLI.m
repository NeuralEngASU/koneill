...
%
%       Usage:
%           [R, PLI, zScore, phaseShift] = CalcPLI(pathName,NEVFileName,tStart,tSpan);
%
%       Outputs:
...
function [R, PLI, zScore, phaseShift] = CalcPLI(pathName,NEVFileName, tStart, tSpan)
...
if tStart == 0
    ...
    idxPLI   = idxStart:(idxSpan+idxStart);
else
...
% % % % % % % % L = NFFT;%length(sig1); % the 'frequency' of the signal. 
% % % % % % % % f = fftshift(( 0:(L-1)) - L/2); % The multiplicative adjustment to adjust phase across all frequencies
...
for ii = spanChannels % Loop over each channel
    for jj = spanComp % Loop over all comparisons (1:32, 2:32, 3:32, ..., 31:32, 32)
        ...
        for kk = 1:surrDataCount % loop over the surrogate data trial
            ...
            surrData = real(ifft(surrData, NFFT));
