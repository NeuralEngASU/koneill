...
%
%       Usage:
%           [R, PLI, zScore, phaseShift] = CalcPLI(pathName,NEVFileName);
%
%       Outputs:
...
function [R, PLI, zScore, phaseShift] = CalcPLIPar(pathName,NEVFileName)
...
% Allocate variables
sig1 = zeros(1,length(C1)); % Signal 1 container
sig2 = zeros(1,length(C1)); % Signal 2 container

numChannels = Header.ChannelCount; % Number of channels
spanChannels = 1:numChannels; % The total channel span
spanComp = spanChannels; % The span of the channel comparisons

surrDataCount = 20; % Number of surrogate data trials
...
% fs and f control how the singal is phase shifted. The actual fs of the
% signal is much different.
fs = length(sig1); % the 'frequency' of the signal. 
f = fftshift(( 0:(fs-1)) - fs/2); % The multiplicative adjustment to adjust phase across all frequencies

tSet = tic; % Set a overall start time
parfor ii = spanChannels % Loop over each channel
    for jj = (ii-1):32 % Loop over all comparisons (1:32, 2:32, 3:32, ..., 31:32, 32)
        ...
        % Save the fft of signal 2 for use in the surrogate data loop
        surrFFT = fft(sig2);
        
        for kk = 1:surrDataCount % loop over the surrogate data trial
            
            tLoop = tic; % Start a loop time counter
            
            % Shift the phase of the surrogate data and calculate the
            % inverse-fft
            surrData = surrFFT.* exp(1i*2*f*phaseShift(ii,jj,kk));
            surrData = real(ifft(surrData));
            
            % Calculate the instantaneous phase of the surrogate data
            ...
            % completed
            if ii == 1
                numComplete = sum(spanComp<jj)*20 + kk;
            elseif ii > 1
                numComplete = sum(spanChannels(end - (ii-2):end))*20 + sum(spanComp<jj)*20 + kk;
            end % END IF ii
            
            numTotal =  sum(1:32)*20; % Calculate the toal number of loops that will occur
            
            timeLeft = (numTotal - numComplete) * timeSpent; % Approximation of the time left to finish code
            timeSpent = numComplete * timeSpent; % Approximation of the time spent running the code so far
        ...
        % Calculate the 'real' instantaneous phase of signal 2
        sig2Phi = atan(imag(hilbert(sig2))./sig2);
        ...
        R(ii,jj,21) = abs(1/length(deltaPhi) * sum( exp(1i * deltaPhi(1:end-1))) );
        PLI(ii,jj,21) = abs(mean(sign(deltaPhi)));
        
        % Compute the zScore for each PLI. A |score| >= 1.96 is above the
        % 95% confidence interval. 
        zScore(ii,jj,:) = zscore(PLI(ii,jj,:) - PLI(ii,jj,21));
