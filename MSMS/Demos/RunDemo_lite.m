...
SimInfo = ParseXML('E:\MSMS\MSMS 2.1\Scenarios\MaleRight');

ParseRelPos( SimInfo, 'Full' );
...
function ParseRelPos( SimInfo, mode )
...
relPos        = zeros(7, (armLength + 4)); % seven rows (xyz,theta,uvw) and columns for anchor point, 3 global rotations, and 8 joints.
...
% relPosRing(:,end)   = [-0.0095; -0.0140; 0.0050; NaN; NaN; NaN; NaN];
% relPosLittle(:,end) = [-0.0095; -0.0140; 0.0050; NaN; NaN; NaN; NaN];
% relPosPalm(:,end)   = [      0;   -0.06;   0.04; NaN; NaN; NaN; NaN];

relPosThumb(:,end)  = [ -0.011;    -.02;    .003; NaN; NaN; NaN; NaN];
relPosIndex(:,end)  = [0.00575;  -0.015; -0.0065; NaN; NaN; NaN; NaN];
relPosMiddle(:,end) = [ 0.0065;  -0.015;  0.0049; NaN; NaN; NaN; NaN];
relPosRing(:,end)   = [-0.0095; -0.0140;  0.0050; NaN; NaN; NaN; NaN];
relPosLittle(:,end) = [-0.0095; -0.0140; -0.0055; NaN; NaN; NaN; NaN];
relPosPalm(:,end)   = [      0;   -0.06;    0.04; NaN; NaN; NaN; NaN];
...
% else
%     relPos(1:3,1) =  [0.450292171549640; -0.186322297363475; -0.0258210774390906;]; % Global Offset
% end

%%% Right
if armLength
    relPos(1:3,1) = [ 0; 0; 0 ]; % Global Offset
else
    relPos(1:3,1) =  [0;0;0]; % Global Offset
end
...
%%% LEFT ARM
% % If we are doing the full arm
% if armLength
%     %               % local rotations
%     relPos(4,5:10) = [0, (45/360)*(2*pi), 0, 0, (45/360)*(2*pi), (-30/360)*(2*pi)];
% else % Else partial arm
%     %                 Global ZYX rotations
%     relPos(4,2:4) = [ -(30/360)*(2*pi), 0, (90/360)*(2*pi)];
% end
    ...
    for j = 1:length(targets)
        ...
        switch finger
            case 1
                ...
                relPos{1}(4,2:4) = [ rot2(2,k), rot2(1,k), 0 ];        
                posPOI = CalculatePos(relPos{1});
...
%                 
            ...
            case 2
                ...
                relPos{2}(4,2:4) = [ rot3(3,k), rot3(2,k), rot3(1,k) ];  
                posPOI = CalculatePos(relPos{2});                 
                %%
                % Place Sphere
...
%                 udpPacket = PlaceSphere(udpPacket, SimInfo, 2, posPOI);
            ...
            case 3
                ...
                relPos{3}(4,2:4) = [ rot3(3,k), rot3(2,k), rot3(1,k) ];  
                posPOI = CalculatePos(relPos{3}); 
...
%                  0.5850, -0.1610, 0.0 ];
    % RIGHT ARM
    location = [ 0.4513, -0.1918, 0.4508;...
                 0.4226, -0.1971, 0.4423;...
                 0.4226, -0.1610, 0.4423 ];
